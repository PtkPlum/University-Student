...
def NewtonOptimizer(F, X, eps=1e-4, Alpha=1, dtype='complex', show=False):
    ...
    while (np.sum((Xnew - Xold) ** 2)) ** .5 > eps:
        ...
        ddf    = np.array([[sp.N(sp.diff(df[j], X[i]).subs(Sub)) for i in range(len(X))] for j in range(len(X))], dtype=dtype)
        df     = np.array([sp.N(df[i].subs(Sub)) for i in range(len(X))], dtype=dtype)
        if np.linalg.det(ddf) == 0:
            print("\n-------------------------------------------------------------------------------")
            print("\tWarning :  f{} = {} is infeasible".format(X, F))
            print("-------------------------------------------------------------------------------\n")
            return Xnew, sp.N(F.subs(Sub))
        invddf = np.linalg.inv(ddf)
        Xold   = Xnew
        Xnew   = Xold -  Alpha * np.dot(invddf, df)
        n += 1
        if show:
            print("\tX{} = {}\n".format(n,Xold))
